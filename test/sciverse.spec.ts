...
import { createSciverseClient } from '../src/sciverse/client.js'
...
describe('SciverseClient (facade over the sciverse SDK)', () => {
  it('sends the Bearer token and posts to /meta-search', async () => {
    ...
    const r = await sc.searchPapers({ query: 'transformer' })
    expect(seen.url).toBe('https://api.sciverse.space/meta-search')
    expect(seen.init?.method).toBe('POST')
    expect((seen.init?.headers as Record<string, string>).authorization).toBe('Bearer sv-secret')
    expect(JSON.parse(String(seen.init?.body)).query).toBe('transformer')
  ...
  it('translates semantic-search mode and GETs content with query params', async () => {
    ...
    expect(body.retrieval).toBe('es') // SDK mode fast -> retrieval=es
  ...
  it('bounds a stalled call with the facade timeout', async () => {
    stubFetch(() => new Promise<Response>(() => {}))
    const sc = createSciverseClient('tk', 60)
    await expect(sc.listCatalog()).rejects.toThrow('timeout after 60ms')
  })

  it('rejects cleanly when the API reports an error status', async () => {
    stubFetch(() => new Response('rate limited', { status: 429 }))
    const sc = createSciverseClient('tk', 5000)
    await expect(sc.searchPapers({})).rejects.toThrow(/429/)
  })
})