...
describe('fetchOne', () => {
  it('returns a clean not_found (never crashes) when no source yields a PDF URL', async () => {
    ...
    const result = await fetchOne(runtime(), '10.1007/s11263-022-01611-x', { checkDns: false })
  ...
  it('downloads from a resolved candidate (Unpaywall hit)', async () => {
    ...
    const result = await fetchOne(runtime({ unpaywallEmail: 'you@example.com' }), '10.1038/s41586-021-03819-2', { checkDns: false })
...
describe('fetchOne web-search fallback', () => {
  it('searches the title and downloads a free PDF after the OA source fails', async () => {
    ...
    const result = await fetchOne({ ...runtime({ unpaywallEmail: 'you@example.com' }), searchWeb } as FetchRuntime, '10.1007/s11263-022-01611-x', { checkDns: false })
    expect(result.success).toBe(true)
  ...
  it('does not use the web fallback when no title is known', async () => {
    ...
    const result = await fetchOne({ ...runtime(), searchWeb } as FetchRuntime, '10.1007/s11263-022-01611-x', { checkDns: false })
    expect(result.success).toBe(false)
    expect(result.error?.code).toBe('not_found')
  ...
  it('skips an existing file BEFORE the web fallback (never overwrites with overwrite:false)', async () => {
    ...
    try {
      ...
      const searchWeb = vi.fn(async () => [{ url: 'https://example.com/found.pdf' }])
      const result = await fetchOne({ ...runtime({ unpaywallEmail: 'you@example.com' }), searchWeb } as FetchRuntime, doi, { checkDns: false })
      expect(result.success).toBe(true)
...
describe('resolveOne web-search fallback', () => {
  it('reports a free PDF URL found by title web-search (no download)', async () => {
    ...
    const result = await resolveOne({ ...runtime({ unpaywallEmail: 'you@example.com' }), searchWeb } as FetchRuntime, '10.1007/s11263-022-01611-x', { checkDns: false })
    expect(result.success).toBe(true)
    expect(result.source).toBe('web_search')
