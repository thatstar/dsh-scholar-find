...
export type LibraryFile = {
  /** Which subdirectory the file lives in (`pdfs`/`md`/`html`/`figs`/`cards`). */
  sub: OutputSubdir
  /** Base filename. */
  file: string
...
export const LIBRARY_SUBS: OutputSubdir[] = ['pdfs', 'md', 'html', 'figs', 'cards']
...
export function pickSubdirs(want: string | undefined): OutputSubdir[] {
  if (want === 'pdfs' || want === 'md' || want === 'html' || want === 'figs' || want === 'cards') return [want]
  return LIBRARY_SUBS
}
