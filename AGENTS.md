...
| `astaApiKeyRef` | Optional Ai2 Asta corpus MCP key as a **DSH credential reference** — the field holds only the **record name** (e.g. `ASTA_API_KEY`); the key itself lives in DSH key management and is never stored in the repo. Enables `scholar_get_paper_content` (~500-word full text). |
