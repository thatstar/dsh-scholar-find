...
import { resolveChain, resolveTitle } from './chain.js';
...
export async function resolveOne(rt, doi) {
    ...
    try {
        ...
        return {
            ...
            error: makeError('resolve_network_error', `Metadata resolvers failed: ${e.message}`),
        };
    }
...
export async function fetchOne(rt, doi, opts = {}) {
    const normalized = normalizeDoi(doi);
    if (!isValidDoi(normalized)) {
        return {
            doi: normalized,
...
export async function fetchBatch(rt, dois, opts = {}) {
    const outDir = resolveOutDir(rt.settings.pdfOutputDir, rt.baseDir);
    ...
    const ok = succeeded === results.length ? true : succeeded === 0 ? false : 'partial';
    ...
    if (opts.idempotencyKey)
        await idemStore(outDir, opts.idempotencyKey, envelope);
    return envelope;
}
/** List PDFs already in the library directory. */
export async function listLibrary(rt) {
    const outDir = resolveOutDir(rt.settings.pdfOutputDir, rt.baseDir);
    let entries;
    try {
...
export async function resolveTitleToDoi(rt, title) {
...
export function previewCandidate(c) {
