...
export function isPrivateIPv4(ip) {
    ...
    const [a, b] = parts;
    if (a === 0 || a === 127 || a === 255)
        return true; // 0/8, loopback, broadcast
    if (a === 10)
        return true; // 10/8
    if (a === 100 && b >= 64 && b <= 127)
...
export async function hostAddrsSafe(host) {
    let addrs;
    try {
...
export async function urlFetchAllowed(url) {
 ...
 */
export async function fetchWithRedirects(url, init, opts = {}) {
    let current = url;
    for (let hop = 0; hop <= (opts.maxRedirects ?? MAX_REDIRECTS); hop++) {
        ...
        if (!gate.ok) {
            const err = new Error(`unsafe fetch target ${current}: ${gate.reason}`);
    ...
    const err = new Error(`too many redirects from ${url}`);
...
export async function readBodyCapped(r, cap, signal) {
    if (!r.body)
        return null;
